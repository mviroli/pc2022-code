...
object MSet {
  ...
  private case class MSetImpl[A](val asMap: Map[A,Int]) extends MSet[A] {
    ...
    override def map[B](f: (A) => B): MSet[B] = new MSetImpl[B](asList.map(f))
    override def flatMap[B](f: (A) => MSet[B]): MSet[B] = new MSetImpl[B](asList.flatMap(x => f(x).asList))
    override def filter(f: (A) => Boolean): MSet[A] = new MSetImpl[A](asList.filter(f))
    override def collect[B](f: PartialFunction[A, B]): MSet[B] = new MSetImpl[B](asList.collect(f))
  }
}