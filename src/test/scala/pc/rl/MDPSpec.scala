...
class RLSpec extends FlatSpec{
  ...
  "QRL" should "properly run an episode" in {
    ...
    assert( (-5 to 10).forall(i => qf.choice(State(i))==left))
  }

  "QRL" should "properly execute A run after learning" in {
    var qf = TwoWaysMDP.rl.qfTW()
    qf = rl.runEpisodes(100,qf)
    assertResult( List((left,State(-1)), (left,State(-2)), (left,State(-3)), (left,State(-4)), (left,State(-5))) )(rl.system.run(qf).toList)
  }
}
